<div>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="box-shadow: 0 0 100vw 100vw #00000080;">
                ...
                <div class="flex justify-between items-start p-5 rounded-t border-b dark:border-gray-600">
                    <h3 id="editUserModalTitle" class="text-xl font-semibold text-gray-900 lg:text-2xl dark:text-white">
                        Halda kasutajat <small class="opacity-75">#</small>{{ $user->id }} {{ $user->name }}
                    </h3>
                ...
                <div class="p-6">
                    <form wire:submit.prevent="save" class="text-gray-900">
                        <fieldset>
                            <legend class="text-lg font-bold dark:text-white mb-3">Load/Rollid:</legend>

                            <div>
                                <div class="flex flex-row">
                                    <x-label for="user.is_admin" :value="__('Kas kasutaja on Admin?')" />
                            ...
                            <div class="mt-3">
                                <div class="flex flex-row">
                                    <x-label for="user.is_author" :value="__('Kas kasutaja on Autor?')" />
                    ...
                    <fieldset class="mt-3">
                        <legend class="text-lg font-bold dark:text-white mb-3">Ohutsoon:</legend>
                        @if(config('blog.bans'))
                        <div class="mb-2">
                            @if($user->is_banned)
    ...
    <script>
        function confirmUserDelete() {
            if (prompt('Kas olete kindel, et soovite selle kasutaja kustutada? Seda ei saa tagasi võtta! Kinnitamiseks kirjutage "kustuta".') == 'kustuta') {
                return true;
            } else {
		        alert('Tühistatud.');
                return false;
