...
class MarkdownFileParser extends Controller
{
    ...
    public static function getQualifiedFilepath(string $filename, bool $validateExistence = true): string {
        ...
        if ($validateExistence && !file_exists($filepath)) {
            throw new Exception("Faili ei leitud", 1);
        }
    ...
    public static function sync(string $filename = null) {
        ...
        return "Sünkroonitud {$count} pilti {$time} sekundiga.";
    }
    ...
    private function validateMatter(array $matter): array
    {
        ...
        try {
            ...
            throw new Exception('Ühtegi kasutajat ei leitud', 1);
        }

        if (!$author) {
            throw new Exception('Kasutajat pole olemas', 1);
        }

        try {
            $tags = explode(', ', $validated['tags']);
        } catch (\Throwable $th) {
            throw new Exception('Kategooriaid ei saanud sõeluda', 1);
        }

        try {
            $published_at = Carbon::parse($validated['published']);
        } catch (\Throwable $th) {
            throw new Exception('Avaldamiskuupäeva ei saanud sõeluda', 1);
        }
        
        try {
            $updated_at = Carbon::parse($validated['updated_at']);
        } catch (\Throwable $th) {
            throw new Exception('Muutmise kuupäeva ei saanud sõeluda', 1);
        }

        try {
            $slug = Str::slug($validated['slug']);
        } catch (\Throwable $th) {
            throw new Exception('Slugi ei saanud luua', 1);
        }
